...
**Client IP / proxy trust.** `auth/local_network._get_client_ip()` is the single source of truth for the caller's address — the local bypass, login and API rate limiting (`routes/auth.py`, `auth/jwt_auth.py`) and `auth/audit.log_action()` all go through it. It returns `request.remote_addr` and never parses forwarded headers itself. `ProxyFix` is installed in `create_app()` **only** when `Config.TRUSTED_PROXY_COUNT` (env `TRUSTED_PROXY_COUNT`, default `0`) is greater than 0, so with the default no client-supplied header can influence `remote_addr`. Even then it is the `auth/proxy_trust.TrustedPeerProxyFix` subclass, which unwinds forwarded headers only when the TCP peer is a trusted proxy (`TRUSTED_PROXY_PEERS`, a CIDR list; unset = loopback/private peers) — a client that reaches gunicorn directly keeps its real address, scheme and host regardless of the headers it sends. When ProxyFix is active, `CF-Connecting-IP` is additionally honoured, but only if the pre-ProxyFix peer (`environ["werkzeug.proxy_fix.orig"]["REMOTE_ADDR"]`) is loopback/private. Deployments behind cloudflared or nginx must set `TRUSTED_PROXY_COUNT=1`; a startup WARNING is logged when CF Access or the local bypass is enabled while it is 0. Independently of that setting, `_came_through_cloudflare()` vetoes the local bypass (both new auto-logins and re-validation of existing bypass sessions) whenever the request carries `CF-Connecting-IP`, `Cf-Ray`, `Cf-Access-Jwt-Assertion` or the `CF_Authorization` cookie — otherwise a cloudflared host on a trusted subnet would auto-login every Cloudflare visitor as admin when `TRUSTED_PROXY_COUNT` is 0. This is fail-closed: forging those markers only denies the bypass. The bypass middleware is registered before the CF Access one, so this veto is what lets CF Access see the request at all.
