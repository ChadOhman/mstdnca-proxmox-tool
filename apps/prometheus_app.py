...
def run_prometheus_upgrade(log_callback=None):
    ...
    if not latest:
        ...
        return False, log_lines
    # Use-time check: the stored version becomes a download URL and an
    # extraction path on the guest, so refuse anything that is not a release
    # tag even if an older release stored it unvalidated.
    try:
        _validate_release_tag(latest, "Prometheus target version")
    except ValueError:
